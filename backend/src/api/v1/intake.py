...
async def chat_turn(
    ...
    logger.info("[INTAKE API Step 1] Received chat turn for campaign_id=%s | user_message='%s'", req.campaign_id, req.user_message)
    ...
    logger.info("[INTAKE API Step 2] Chat turn complete for campaign_id=%s | Reply='%s' | Complete=%s", req.campaign_id, res.get("reply"), res.get("is_complete"))
...
async def get_intake_history(
    ...
    """Fetch persistent chat history and current checklist for a campaign."""
...
async def migrate_intake_session(
    ...
    logger.info("[INTAKE MIGRATE Step 1] Starting migration from session_id=%s -> campaign_id=%s", req.session_id, req.campaign_id)
    ...
    try:
        ...
        checklist_res = repo.client.table("intake_checklists").select("*").eq(
            "campaign_id", str(req.session_id)
        ).execute()
    except Exception as e:
        logger.error("[INTAKE MIGRATE ERROR] Could not load intake checklist for session_id=%s: %s", req.session_id, e)
        raise HTTPException(status_code=500, detail=f"Could not load intake checklist: {e}")

    if not checklist_res.data:
        logger.warning("[INTAKE MIGRATE WARNING] No intake checklist found for session_id=%s", req.session_id)
    ...
    try:
        logger.info("[INTAKE MIGRATE Step 3] Upserting checklist under real campaign_id=%s", req.campaign_id)
        ...
        logger.info(
            ...
            req.session_id, req.campaign_id,
            checklist_row.get("event_name"), checklist_row.get("guest_name"),
        )
    except Exception as e:
        logger.error("[INTAKE MIGRATE ERROR] Failed to upsert checklist under campaign_id=%s: %s", req.campaign_id, e)
        raise HTTPException(status_code=500, detail=f"Could not migrate checklist: {e}")

    # 3. Migrate message history (best-effort)
    try:
        logger.info("[INTAKE MIGRATE Step 4] Migrating chat messages for session_id=%s", req.session_id)
        msg_res = msg_repo.client.table("intake_messages").select("*").eq(
            "campaign_id", str(req.session_id)
        ).execute()
        if msg_res.data:
            migrated = []
        ...
        logger.info("[INTAKE MIGRATE Step 4 SUCCESS] Migrated %d messages %s -> %s", len(msg_res.data or []), req.session_id, req.campaign_id)
    except Exception as e:
        logger.warning("[INTAKE MIGRATE WARNING] Could not migrate intake messages (non-fatal): %s", e)

    # 4. Delete old session row
    try:
        logger.info("[INTAKE MIGRATE Step 5] Cleaning up old temporary session_id=%s", req.session_id)
        repo.client.table("intake_checklists").delete().eq(
            "campaign_id", str(req.session_id)
        ).execute()
        logger.info("[INTAKE MIGRATE Step 5 SUCCESS] Deleted old session row for session_id=%s", req.session_id)
    except Exception as e:
...
async def confirm_guest(
    ...
    logger.info("[GUEST CONFIRM Step 1] Guest confirmation received for campaign_id=%s | guest_name='%s' | confirmed=%s", req.campaign_id, req.guest_name, req.confirmed)
    ...
    if req.confirmed:
        logger.info("[GUEST RESEARCH Step 2] Starting DuckDuckGo web search for guest '%s'...", req.guest_name)
        try:
            from src.modules.workflow_engine.graphs.guest_research import guest_research_graph

            logger.info("[GUEST RESEARCH Step 2] Starting autonomous LangGraph guest research agent...")

            # Build context to force the LLM to write a relevant bio
            context_parts = []
            if checklist.event_name: context_parts.append(f"Event: {checklist.event_name}")
            if checklist.category: context_parts.append(f"Category: {checklist.category}")
            if checklist.curriculum_breakdown: context_parts.append(f"Topic/Curriculum: {checklist.curriculum_breakdown}")
            if checklist.outcome_deliverable: context_parts.append(f"Outcome: {checklist.outcome_deliverable}")
            campaign_context = " | ".join(context_parts)

            # Run the autonomous graph
            final_state = await guest_research_graph.ainvoke({
                "guest_name": req.guest_name,
                "guest_title": req.guest_title or "",
            ...
            if "guest_profile" in final_state and final_state["guest_profile"]:
                checklist_dict["guest_profile"] = final_state["guest_profile"]
                logger.info("[GUEST RESEARCH SUCCESS] Researched guest bio completed autonomously.")
            else:
                logger.warning("[GUEST RESEARCH WARNING] Agent finished but no profile was generated.")
        except Exception as e:
            logger.warning("[GUEST RESEARCH ERROR] Could not execute guest web research on confirm: %s", e)
    ...
    logger.info("[GUEST CONFIRM Step 4 SUCCESS] Saved updated checklist with guest research for campaign_id=%s", req.campaign_id)
