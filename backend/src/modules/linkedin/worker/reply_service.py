...
class ReplyService:
    ...
    async def send_manual_reply(self, user_id: str, local_comment_id: str, reply_text: str) -> dict[str, Any]:
        """Send a manual reply to an inbound comment."""
        
        # 1. Fetch local comment and verify ownership
        res = self.supabase.table("linkedin_comments").select("*").eq("id", local_comment_id).eq("user_id", user_id).execute()
        ...
        acc_res = self.supabase.table("linkedin_accounts").select("unipile_account_id").eq("id", account_id).execute()
        if not acc_res.data:
        ...
        try:
            async with httpx.AsyncClient(timeout=15.0) as client:
                ...
                if res.status_code in (200, 201):
                    ...
                    return {"status": "success", "reply_id": reply_record_id, "remote_id": remote_reply_id}
                else:
                    self.supabase.table("linkedin_replies").update({
                        "status": "FAILED"
                    }).eq("id", reply_record_id).execute()
