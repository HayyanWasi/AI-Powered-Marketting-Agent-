...
async def _trigger_session(
    account_id: str, timezone: str, session: SessionWindow, invite_limit: int, engage_limit: int
) -> None:
    ...
    asyncio.create_task(
        executor.execute_session(
            session, daily_invite_limit=invite_limit, daily_engage_limit=engage_limit
        )
    )
...
def start_linkedin_scheduler() -> None:
    ...
    if not scheduler.running:
        # Schedule the daily planner to run at 1:00 AM
        scheduler.add_job(
        ...
        # On startup: trigger planning immediately so mid-day restarts don't lose today's plan
        ...
        logger.info(
            "LinkedIn Scheduler started — post publisher runs every 5 min, daily planner at 01:00."
        )
