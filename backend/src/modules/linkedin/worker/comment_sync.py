...
class CommentSyncWorker:
    ...
    async def sync_all_accounts(self) -> None:
        ...
        try:
            res = self.supabase.table("linkedin_accounts").select("*").eq("status", "connected").execute()
    ...
    async def sync_account(self, account: dict[str, Any]) -> None:
        ...
        res = self.supabase.table("linkedin_posts").select("id, unipile_post_id").eq("linkedin_account_id", account_id).not_.is_("unipile_post_id", "null").execute()
        posts = res.data or []
        ...
        for p in posts:
            ...
            try:
                await self.sync_post_comments(user_id, account_id, unipile_account_id, own_provider_id, social_id)
            except Exception as e:
                logger.error("Failed to sync comments for post %s: %s", social_id, e)

    async def sync_post_comments(self, user_id: str, account_id: str, unipile_account_id: str, own_provider_id: str, social_id: str) -> None:
        ...
        async with httpx.AsyncClient(timeout=15.0) as client:
            ...
            for comment in items:
                ...
                await self._persist_comment(user_id, account_id, social_id, own_provider_id, comment)
                
                # Check for replies
                reply_counter = comment.get("reply_counter")
                if reply_counter and isinstance(reply_counter, int) and reply_counter > 0:
                    reply_params = {"account_id": unipile_account_id, "limit": 100, "comment_id": remote_comment_id}
                    reply_res = await client.get(url, headers=headers, params=reply_params)
                    if reply_res.status_code == 200:
                        reply_data = reply_res.json()
                        reply_items = reply_data.get("items", []) if isinstance(reply_data, dict) else reply_data
                        for reply in reply_items:
                            if reply.get("id"):
                                await self._persist_comment(user_id, account_id, social_id, own_provider_id, reply, parent_id=remote_comment_id)
                                
    async def _persist_comment(self, user_id: str, account_id: str, social_id: str, own_provider_id: str, comment: dict, parent_id: str = None) -> None:
        remote_comment_id = comment.get("id")
        author_details = comment.get("author_details", {})
        author_id = author_details.get("id")
        
        author_str = comment.get("author")
        author_name = author_details.get("name") or (author_str if isinstance(author_str, str) else None)
