...
async def test_linkedin_generation_persistence(mock_get_plan, mock_outreach, mock_posts, mock_freshness, user_a_client):
    mock_posts.return_value = [
        LinkedInPost(campaign_id=uuid.uuid4(), slot_id="1", scheduled_at=datetime.now(UTC), hook="Hook", body="Body", cta_text="CTA", full_content="Full", status=PostStatus.DRAFT)
    ]
    mock_outreach.return_value = OutreachTemplate(campaign_id=uuid.uuid4(), step_invite_msg="Invite")
            ...
            class ClientWrapper:
                ...
                def rpc(self, rpc_name, args):
                    if rpc_name == "replace_linkedin_campaign_content":
                        ...
                        real_client.table("linkedin_posts").delete().eq("campaign_id", c_id).execute()
                        real_client.table("outreach_sequences").delete().eq("campaign_id", c_id).execute()
                        seq_record = {**args["p_sequence"], "campaign_id": c_id}
                        inserted_seq = real_client.table("outreach_sequences").insert(seq_record).execute().data[0]
                        posts_to_insert = [{**p, "campaign_id": c_id} for p in args["p_posts"]]
                        inserted_posts = real_client.table("linkedin_posts").insert(posts_to_insert).execute().data
                        return SimpleNamespace(execute=lambda: SimpleNamespace(data={"posts": inserted_posts, "sequence": inserted_seq}))
    ...
    with patch("src.api.v1.linkedin.BaseRepository", new=MockRepo):
        ...
        res = await client.post("/api/v1/company", json={
        ...
        res = await client.post("/api/v1/campaigns", json={
            ...
            "schedule": {"start_date": "2026-10-01T00:00:00Z", "end_date": "2026-10-31T00:00:00Z", "timezone": "UTC"}
        })
        assert res.status_code == 201, res.text
        ...
        mock_plan = CampaignPlan(
            ...
            status="Approved",
            channel_plan=ChannelPlan(calendar_slots=[
