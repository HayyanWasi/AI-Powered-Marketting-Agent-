...
def mock_user_a() -> AuthenticatedUser:
    return AuthenticatedUser(id="00000000-0000-0000-0000-00000000000A", roles=["user"], permissions=[])
...
def mock_repo(monkeypatch) -> MagicMock:
    ...
    def mock_create(company_name, brand_guidelines, brand_tone=None):
        profile = CompanyProfile(company_name=company_name, brand_guidelines=brand_guidelines, brand_tone=brand_tone)
    ...
    from src.api.v1.company import _get_create_service, _get_update_service, _get_get_service
    ...
    app.dependency_overrides[_get_create_service] = lambda: CreateCompanyService(repository=repo_mock)
    app.dependency_overrides[_get_update_service] = lambda: UpdateCompanyService(repository=repo_mock)
    app.dependency_overrides[_get_get_service] = lambda: GetCompanyService(repository=repo_mock)
    
    return repo_mock

@pytest.fixture
def user_a_client() -> Generator[TestClient, None, None]:
...
def test_update_company_preserves_unknown_fields(user_a_client, mock_repo):
    ...
    resp = user_a_client.post(
        "/api/v1/company",
        json={
            "company_name": "RoundTrip Test",
            "brand_guidelines": json.dumps(initial_guidelines)
        }
    )
    ...
    save_resp = user_a_client.put(
        ...
        json={"brand_guidelines": json.dumps(edit_guidelines)}
    )
