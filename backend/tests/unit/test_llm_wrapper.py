...
def test_retry_wrapper_calls_generate_correctly():
    ...
    service._ordered_chain = MagicMock(return_value=[("mock_provider", mock_provider, "mock-model")])
    ...
    with patch("src.services.llm_service._call_provider_generate", return_value=mock_response) as mock_call:
